...
foreach ($productos as $producto) {
    ...
    <div class="container mt-5">
        ...
        <div class="card mb-4">
            <div class="card-body">
                <form action="accion/acc_editar_producto.php?id=<?= $producto->getId(); ?>" method="post"
                      enctype="multipart/form-data">
                    <div class="mb-3">
                        <label for="nombre" class="form-label">Nombre</label>
                        <input type="text" class="form-control" id="nombre" name="nombre"
                               value="<?= $producto->getProducto_nombre(); ?>">
                    </div>
                    <div class="mb-3">
                        <label for="descripcion" class="form-label">Descripción</label>
                        <textarea class="form-control" id="descripcion"
                                  name="descripcion"><?= $producto->getProducto_descripcion(); ?></textarea>
                    </div>
                    <div class="mb-3">
                        <label for="precio" class="form-label">Precio</label>
                        <input type="text" class="form-control" id="precio" name="precio"
                               value="<?= $producto->getProducto_precio(); ?>">
                    </div>

                    <div class="mb-3">
                        <label for="categoria_id" class="form-label">Categoría</label>
                        <select class="form-select" id="categoria_id" name="categoria_id">
                            <option value="" <?php echo is_null($categoriaId) ? 'selected' : ''; ?>>Seleccione una
                                categoría
                            </option>
                    ...
                    <div class="col-md-2 mb-3">
                        ...
                        <img src="../img/productos/<?= $producto->getProducto_imagen() ?>"
                             alt="Imágen Illustrativa de <?= $producto->getProducto_nombre() ?>"
                             class="img-fluid rounded shadow-sm d-block">
                        <input class="form-control" type="hidden" id="imagen_og" name="imagen_og"
                               value="<?= $producto->getProducto_imagen() ?>">
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="stock" name="stock"
                               value="<?= $producto->getProductoStock(); ?>">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Estado</label>
                        <div>
                            <input type="radio" id="activo" name="estado"
                                   value="1" <?= $producto->getProductoEstado() == 1 ? 'checked' : ''; ?>>
                            <label for="activo">Activo</label>
                        </div>
                        <div>
                            <input type="radio" id="inactivo" name="estado"
                                   value="0" <?= $producto->getProductoEstado() == 0 ? 'checked' : ''; ?>>
                    ...
                    <div class="mb-3">
                        ...
                        <div>
                            <input type="radio" id="destacado" name="destacado"
                                   value="1" <?= $producto->getProductoDestacado() == 1 ? 'checked' : ''; ?>>
                            <label for="destacado">Destacado</label>
                        </div>
                        <div>
                            <input type="radio" id="no_destacado" name="destacado"
                                   value="0" <?= $producto->getProductoDestacado() == 0 ? 'checked' : ''; ?>>
